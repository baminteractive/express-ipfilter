...
module.exports = function ipfilter(ips, opts) {
  ...
  var settings = _.defaults(opts || {}, {
    ...
    customMessage: "Access denied to the IP address: "
  });
  ...
  return function (req, res, next) {
    ...
    if (matchClientIp(ip, req)) {
      ...
      if (settings.log && settings.logLevel !== 'deny') {
        settings.logF(settings.customMessage + ip);
      }
    ...
    if (settings.log && settings.logLevel !== 'allow') {
     ...
     // stacktrace check
      if(settings.disableStackTrace){
      res.status(401);
      res.end(settings.customMessage);
    } else{
        var err = new IpDeniedError(settings.customMessage + ip);
        return next(err);
    }
  };
};
